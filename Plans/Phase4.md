...
Admission logic:
Use max_audit_urls from job config
Default is 10
Admission should select only top 10 URLs by:
homepage / first
then increasing path_depth

ELIGIBLE -> ADMITTED for top-N under caps
rest -> DEFERRED with reason (cap hit / overload)
...
DoD:
calling lease repeatedly returns tasks
...
- On job start, admit URLs: homepage + shallow paths first, up to max_audit_urls and max_path_depth caps
...
- Implement POST /tasks/{id}/complete to store lighthouse_json (jsonb), extracted scores, duration, and mark run/task statuses.
